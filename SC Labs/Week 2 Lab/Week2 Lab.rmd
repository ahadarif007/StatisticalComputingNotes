...
- **Binomial** requires each draw to be **independent** — the
  probability of drawing a land card must stay the same every time.
- But here we're drawing **without replacement** — once you draw a
...
|---|---|
| `dhyper(x, m, n, k)` | P(exactly x successes) — the probability of drawing exactly `x` land cards |
| `phyper(q, m, n, k)` | P(X ≤ q) — the cumulative probability (at most `q` land cards) |
...
- **P(exactly 3 land):** The most common outcome — you'll get exactly 3
  lands in about 26% of hands.
- **P(more than 3 land):** About 17% of the time you'll "flood" with
  too many lands.
- **P(between 2 and 4 land):** The "sweet spot" — about 74% of hands
  will give you a reasonable number of lands.
- **Expected value:** On average, you'll draw 7 × (20/60) ≈ **2.33**
  land cards per hand.
...
more — the real answer is shockingly low!
...
question is about *any* pair — and with 23 people there are
...
- **Expected value** $E[X]$ — the long-run average (what you'd get if
  you repeated the experiment millions of times).
- **Variance** $\text{Var}(X)$ — how spread out the values are around
  the expected value.
...
- $E[X] = np = 12 \times 0.2 = 2.4$ — On average, 2.4 successes out
  of 12 trials.
...
- $E[A] = \mu = 10$ — The centre of the bell curve.
...
- $E[B] = \lambda = 4.5$ — On average, 4.5 events per interval.
- $\text{Var}(B) = \lambda = 4.5$ — (For Poisson, the variance always
  equals the mean!)
...
Yes! The Poisson approximation (≈ 0.003066) is very close to the exact
Binomial answer (≈ 0.002898). With $n = 100$ and $np = 1$, we're well
within the "excellent approximation" zone.

---

# 5. Sampling Distribution — The Central Limit Theorem
...
> curve) distribution — even if the original data is not Normal at
> all!**
...
NOT a bell curve — it's flat!

```{r}
a <- runif(10^6)
# a [ too much big data to print ]
```
...
> **What to see:** The histogram is **flat** — this is the Uniform
...
> **What `dim(b)` tells us:** The result is a 100 × 1000 matrix — each
...
### Step 4: Plot the sample means — it looks Normal!
...
# 6. Plotting Expected Values — The Law of Large Numbers
...
> **What to see:** With 10 flips, the proportion might be 0.3 or 0.7 —
...
For any Normal distribution, we can compute probabilities — i.e., "what
fraction of the bell curve falls to the left of a given value?"

## The `pnorm()` function — your best friend

`pnorm()` computes the **cumulative probability** $P(X \leq x)$ — the
...
| `sd` | The standard deviation — how wide the bell is (default: 1) |
| `lower.tail` | If `TRUE` (default): returns P(X ≤ q). If `FALSE`: returns P(X > q) |
...
### Example 1: P(X < 68) — "What fraction of women are shorter than 68 inches?"
...
### Example 2: P(X > 68) — "What fraction of women are TALLER than 68 inches?"
...
### Example 3: P(62 < X < 68) — "What fraction of women are between 62 and 68 inches?"

We compute: P(X < 68) − P(X < 62) = the area *between* 62 and 68.
...
> mean, and 62 to 68 is exactly μ ± σ = 65 ± 3.
...
### Question 1: P(X < 650) — "What fraction of students score below 650?"
...
> That means scoring 650 puts you in the top 7% — quite impressive!

### Question 2: P(X > 650) — "What fraction score ABOVE 650?"
...
### Question 3: P(450 < X < 650) — "What fraction score between 450 and 650?"
...
`qnorm()` is the **inverse** of `pnorm()` — you give it a probability
...
| `pnorm(q, mean, sd)` | Area to the **left** of q: P(X ≤ q) | `pnorm(68, 65, 3)` |
...
We want $P(D \mid T)$ — the probability of disease GIVEN a positive
test. Bayes' Theorem says:

$$P(D \mid T) = \frac{P(T \mid D) \cdot P(D)}{P(T)}$$

But we need $P(T)$ — the overall probability of testing positive. We
...
- **9,900** don't have the disease → **495** test positive (false
  positives!)
...
- $P(A \mid B) = \frac{P(A \cap B)}{P(B)}$ — "Of the times B happens,
  how often does A also happen?"
- $P(B \mid A) = \frac{P(A \cap B)}{P(A)}$ — "Of the times A happens,
...
- $P(A \mid B) = \frac{0.1}{0.4} = 0.25$ — Given B occurred, there's a
  25% chance A also occurred.
- $P(B \mid A) = \frac{0.1}{0.2} = 0.5$ — Given A occurred, there's a
  50% chance B also occurred.
- **Independence:** $P(A) \times P(B) = 0.08$, but $P(A \cap B) =
  0.1$. Since $0.08 \neq 0.1$, A and B are **NOT independent** —
...
- $P(O \mid U) = 0.4$ — If you've seen a UFO, there's a 40% chance you
  live in the outback.
- $P(O \mid U^C) = 0.2$ — If you haven't seen a UFO, there's a 20%
  chance you live in the outback.
- $P(U) = 0.01$ — The overall probability of seeing a UFO is 1%.

**Question:** What is $P(U \mid O)$ — the probability of seeing a UFO,
...
you've seen a UFO — slightly higher than the base rate of 1%, because
outback dwellers are more likely to report UFO sightings (perhaps
because of darker skies and less light pollution! 🛸).
